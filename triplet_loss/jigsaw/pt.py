...
# -----------------------------------------------------------------------------
# 6) Model: ResNet50 + Triplet Embedding Head
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 7) Triplet Loss with All-Triplet Mining
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 10) Training Loop with Encoder Checkpoints Every 10 Epochs
# -----------------------------------------------------------------------------
num_epochs = 200
...
for epoch in range(100, num_epochs + 1):
    ...
    for (x_i, x_j), _ in pbar:
        ...
        inputs = torch.cat([x_i, x_j], dim=0)                      # [2B, C, H, W]
        B = x_i.size(0)
        labels = torch.arange(B, device=device).repeat(2)         # [2B]

        optimizer.zero_grad()
        embeddings = model(inputs)                               # [2B, 128]
...
# -----------------------------------------------------------------------------
# 11) Save Final Full Model Checkpoint
# -----------------------------------------------------------------------------
